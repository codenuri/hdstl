...
// 결국 std::sort 를 사용할때는
1. std::sort(x, x + 10);		// 비교함수를 전달하지 않으므로 오름차순
2. std::sort(x, x + 10, 비교를 위한 2항 함수);    // 인라인 치환 안됨
3. std::sort(x, x + 10, 비교를 위한 2항 함수객체); // 인라인 치환 됨
4. std::sort(x, x + 10, 비교를 위한 람다 표현식);  // 인라인 치환 됨
// 3번과 4번은 동일한 결과 입니다.
